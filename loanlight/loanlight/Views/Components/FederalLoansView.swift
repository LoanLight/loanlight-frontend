...
struct FederalLoansView: View {
                ...
                ScrollView(showsIndicators: false) {
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        if !viewModel.confirmedLoans.isEmpty {
                            confirmedLoansSection.padding(.bottom, 20)
                        }

                        uploadCard.padding(.bottom, 12)
                        importDirectlyCard
                        Spacer(minLength: 40)
                    }
                    .padding(.horizontal, 24)
    ...
    private func submitFederalLoans() {
        ...
        Task {
            ...
            for loan in payload.loans {
                print("[FederalLoans] \(loan.loanName): balance=\(loan.balance) rate=\(loan.interestRate)")
            }
            #endif

            do {
                // Use `let _: Type =` NOT `... as Type` — required for correct generic inference
                let _: FederalLoanBulkOut = try await APIClient.post(
                    path: "/federal-loans/bulk",
                    body: payload
                )
                await MainActor.run { onComplete(viewModel.confirmedLoans) }
            } catch {
                await MainActor.run {
                    viewModel.errorMessage = (error as? LocalizedError)?.errorDescription
                        ?? "Could not save federal loans. Please try again."
                }
            }
        }
    }
    ...
    private var stepPill: some View {
        HStack(spacing: 6) {
            Circle().frame(width: 6, height: 6).foregroundColor(.sage)
            Text("STEP \(currentStep) OF \(totalSteps)")
                .font(AppFont.microBold).foregroundColor(.sage).kerning(0.5)
        }
    ...
    private var confirmedLoansSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("ADDED LOANS")
                .font(AppFont.sectionLabel).tracking(1.2).foregroundColor(.mist)

            ForEach(viewModel.confirmedLoans) { loan in
                HStack(spacing: 12) {
                    Image(systemName: "checkmark.circle.fill")
                        .foregroundColor(.success).font(.system(size: 18))
                    VStack(alignment: .leading, spacing: 2) {
                        Text(loan.loanType.isEmpty ? "Federal Loan" : loan.loanType)
                            .font(AppFont.bodySemibold).foregroundColor(.ink)
                        Text([loan.totalBalance, loan.interestRate]
                            .filter { !$0.isEmpty }.joined(separator: " · "))
                            .font(AppFont.caption).foregroundColor(.mist)
                    }
                    Spacer()
                    Button(action: { viewModel.confirmedLoans.removeAll { $0.id == loan.id } }) {
                        Image(systemName: "xmark.circle.fill")
                            .foregroundColor(.mist).font(.system(size: 16))
                    }
                }
    ...
    private var uploadCard: some View {
        Button(action: { showFilePicker = true }) {
            VStack(spacing: 12) {
                ZStack {
                    RoundedRectangle(cornerRadius: 12).fill(Color.sagetint).frame(width: 52, height: 52)
                    Image(systemName: "doc.fill").font(.system(size: 22))
                        .foregroundColor(Color.sage.opacity(0.7))
                }
                Text("Upload FAFSA PDF").font(AppFont.bodySemibold).foregroundColor(.ink)
                Text("From StudentAid.gov or your servicer")
                    .font(AppFont.caption).foregroundColor(.mist).multilineTextAlignment(.center)
                Text("PDF").font(AppFont.microBold).foregroundColor(.mist)
    ...
    private var importDirectlyCard: some View {
        VStack(spacing: 0) {
            HStack {
                Text("OR IMPORT DIRECTLY").font(AppFont.microBold).foregroundColor(.mist).kerning(0.5)
                Spacer()
            }
            ...
            HStack(spacing: 10) {
                ...
                Text("Connect StudentAid.gov").font(AppFont.bodyMedium).foregroundColor(.ink)
                Spacer()
                Image(systemName: "chevron.right").font(.system(size: 13, weight: .medium)).foregroundColor(.mist)
            }
    ...
    private var bottomButtons: some View {
        VStack(spacing: 10) {
            Button(action: { showFilePicker = true }) {
                Text("Upload PDF").font(AppFont.ctaButton).foregroundColor(.white)
                    .frame(maxWidth: .infinity).padding(.vertical, 17)
                    .background(Color.sage).clipShape(RoundedRectangle(cornerRadius: 14))
            }

            Button(action: {
                viewModel.extractedLoan = LoanEntity()
                viewModel.showConfirmation = true
            }) {
                Text("Enter Manually").font(AppFont.ctaButton).foregroundColor(.ink)
                    .frame(maxWidth: .infinity).padding(.vertical, 17)
                    .background(RoundedRectangle(cornerRadius: 14).stroke(Color.border, lineWidth: 1.5))
            }
                ...
                Button(action: { submitFederalLoans() }) {
                    HStack(spacing: 8) {
                        if isSubmitting {
                            ...
                            Text("Continue").font(AppFont.ctaButton)
                            Image(systemName: "arrow.right").font(.system(size: 14, weight: .bold))
                        }
                    }
                    .foregroundColor(.ink).frame(maxWidth: .infinity).padding(.vertical, 17)
                    .background(Color.gold).clipShape(RoundedRectangle(cornerRadius: 14))
                }
    ...
    private var loadingOverlay: some View {
        ZStack {
            ...
            VStack(spacing: 16) {
                ...
                Text("Extracting loan data…").font(AppFont.captionBold).foregroundColor(.white)
            }
            .padding(32)
            .background(RoundedRectangle(cornerRadius: 20).fill(Color.ink.opacity(0.9)))
        }
    }
}

#Preview {
    FederalLoansView(currentStep: 1, totalSteps: 7)
}
